...
public class ConeRays extends Game {

    @Override
    public Screen getStartScreen() {
        return new ConeRaysScreen(this);
    }




    class ConeRaysScreen extends Screen {
        ...
        ConeRaysScreen(Game game) {
            ...
            opening = new Vector3(1f,0.3f,0).nor();
            ...
            i = new Vector3(rangeProjOnDir).sub(opening);
            radius = i.len();
            i.nor();
            ...
            float r = radius / rangeProjOnDir.len() * range * density;
            int rayCount = (int) (r * r * (float) Math.PI);

            coordinates = new float[2 * 7 * rayCount + 4 * 7/*extra 2 for direction and range*/];
            ...
            for (int l = 0; l < rayCount; l++) {
                ...
                float dist = (float) Math.sqrt(l /(rayCount - 1f)) * radius;

                x.set(i).mul(dist * (float) Math.cos(angle * l));
                y.set(j).mul(dist * (float) Math.sin(angle * l));
                ray.set(rangeProjOnDir.x + x.x + y.x,
                        rangeProjOnDir.y + x.y + y.y,
                        rangeProjOnDir.z + x.z + y.z).nor().mul(range);
            ...
            System.out.println("Total of " + rayCount + " rays casted within: "
                    + deltaTime + " Second");
